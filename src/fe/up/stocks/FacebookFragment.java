...
public class FacebookFragment extends Fragment {
	...
	private stockGraph facebook;
	private realtimeGraph facebook2;
			...
			protected View doInBackground(View... params) {
				...
				for(int i=0; i< MainActivity.sgraph.size();i++)
				{
					if(sg.stockName.equals(MainActivity.sgraph.get(i).stockName))
					{
						...
						{
							exists=true;
							facebook = MainActivity.sgraph.get(i);
							//facebook2 = MainActivity.sgraph.get(i);
							graphPos=i;
							break;
						}
					}
				...
				if(!exists)
				  ...
				  if(facebook.points.size()<1)
				  {
				    ...
				    try {
				    	  ...
				    	  int bmonth = facebook.beginDate.get(Calendar.MONTH);
				    	  int bday = facebook.beginDate.get(Calendar.DAY_OF_MONTH);
				    	  int byear = facebook.beginDate.get(Calendar.YEAR);
				    	  
				    	  int emonth = facebook.endDate.get(Calendar.MONTH);
				    	  int eday = facebook.endDate.get(Calendar.DAY_OF_MONTH);
				    	  int eyear = facebook.endDate.get(Calendar.YEAR);
				          // Build RESTful query (GET)
				    	  URL url;
				    	  if(MainActivity.monthPeriodGraph)
				    		  url = new URL("http://ichart.finance.yahoo.com/table.txt?a="+bmonth+"&b="+bday+"&c="+byear+"&d="+emonth+"&e="+eday+"&f="+eyear+"&g=d&s="+facebook.stockAbrev);
				    	  else
					          url = new URL("http://ichart.finance.yahoo.com/table.txt?a="+bmonth+"&b="+bday+"&c="+byear+"&d="+emonth+"&e="+eday+"&f="+eyear+"&g=m&s="+facebook.stockAbrev);
				...
				if (pd!=null) {
					...
					switch(response) {
						...
						default:
						{
							...
							int k=4;
							int position=5;
							facebook.pos=position;
							for (int j=0; j<position; j++) {
								long x=facebook.points.get(j).second;
								double y=facebook.points.get(j).first;
								data[k] = new GraphViewData(x, y); // next day
								k--;
							}
							totaldiff = (facebook.maxY-facebook.minY)/3;
							...
							seriesStyle1.setValueDependentColor(new ValueDependentColor() {
								...
								public int get(GraphViewDataInterface data) {
									if(data.getY()<= (facebook.minY+totaldiff*1))
										return Color.rgb(0, 50, 150);
									else if(data.getY()<= (facebook.minY+totaldiff*2))
							...
							graphView.setManualYAxisBounds(facebook.maxY, facebook.minY);
							/*
							...
							graphView.setCustomLabelFormatter(new CustomLabelFormatter() {
								...
								public String formatLabel(double value, boolean isValueX) {
									if (isValueX) {
										if(facebook.monthPeriod)
											return dateFormat.format(facebook.times.get((int) value));
										else
											return dateYearFormat.format(facebook.times.get((int) value));
									}
									else{
										double d= value;
							...
							MainActivity.graphs.add(graphView);
							facebook.graphPos=MainActivity.graphs.size()-1;
							LL3.addView(MainActivity.graphs.get(MainActivity.graphs.size()-1));
									...
									public void onClick(View v) {
										...
										View parent = (View)p.getParent();
					    					stockGraph sg = MainActivity.sgraph.get(graphPos);
					        					...
					        					ln.removeAllViews();
					        					MainActivity.graphs.set(facebook.graphPos,graphView);
					        					ln.addView(MainActivity.graphs.get(facebook.graphPos));

					        					MainActivity.graphs.get(facebook.graphPos).startAnimation(animation);
					        				}
					        				//Log.i("test", "got here2");
									}
						        });
					        ...
					        title.setText(facebook.stockName);
							...
							LL.addView(LL2);
								...
								{
									if(facebook.stockName.equals(MainActivity.rgraph.get(i).stockName))
									{
										{
											exists=true;
											facebook2 = MainActivity.rgraph.get(i);
											//facebook2 = MainActivity.sgraph.get(i);
											graphPos2=i;
											graphView2 = facebook2.gvs;
								...
								if(!exists)
									{
										...
										/*for (int j=0; j<31; j++) {
											long x=0;
											double y=facebook.points.get(facebook.points.size()-1).first;
											facebook2.points.add(new Pair<Double,Long>(y,x));
										}*/
										data = new GraphViewData[0];
										//position=5;
										facebook2.pos=0;
										/*for (int j=0; j<31; j++) {
											long x=facebook2.points.get(j).second;
											double y=facebook2.points.get(j).first;
											data[j] = new GraphViewData(x, y); // next day
											//k--;
										}*/
									    graphView2 = new GraphViewSeries(data);
									    graphView2.getStyle().color = Color.RED;
									    facebook2.maxY=(double) facebook.points.get(facebook.points.size()-1).first;
										facebook2.minY=(double) 0;
									}

								// graph with custom labels and drawBackground
									graphView = new LineGraphView(
											getActivity()
											, ""/*facebook2.stockName+" Realtime Graph"*/
									);
									//((LineGraphView) graphView).setDrawBackground(true);
									
									
								graphView.addSeries(graphView2); // data
								facebook2.gvs = graphView2;
								MainActivity.rgraph.set(graphPos2, facebook2);
								graphView.setViewPort(facebook2.pos, 4);
								...
								graphView.setManualYAxisBounds(facebook2.maxY, facebook2.minY);
								
								graphView.setCustomLabelFormatter(new CustomLabelFormatter() {
									@Override
									public String formatLabel(double value, boolean isValueX) {
										if (isValueX) {
											if(value<0 || value>=facebook2.times.size())
												{
													Log.d(Double.toString(value),Integer.toString(facebook2.times.size()));
													return "0:00";
												}
											else
												return facebook2.times.get((int) value);
										}
										else{
													double d= value;
													String text = Double.toString(Math.abs(d));
								...
								MainActivity.graphs.add(graphView);
								facebook2.graphPos=MainActivity.graphs.size()-1;
										...
										public void onClick(View v) {
											if(facebook2.isPlay)
											 ...
											 if(!facebook2.isPlay){
												 	...
												 	facebook2.isPlay=true;
												 	MainActivity.rgraph.set(graphPos2,facebook2);
											       }
											       else{
											    	   facebook2.isPlay=false;
						        ...
						        btnR4.setOnClickListener(new OnClickListener() {
									...
									public void onClick(View v) {
									    ...
									    graphView2 = new GraphViewSeries(data);
									    graphView2.getStyle().color = Color.RED;
									    facebook2.points= new ArrayList<Pair<Double,Long>>();
									    MainActivity.rgraph.set(graphPos2,facebook2);
									    facebook2.maxY=(double) facebook.points.get(facebook.points.size()-1).first;
										facebook2.minY=(double) 0;
										facebook2.times=new ArrayList<String>();
										//MainActivity.graphs.get(facebook2.graphPos).removeAllSeries();
				    					//MainActivity.graphs.get(facebook2.graphPos).addSeries(graphView2);
									...
									graphView.setCustomLabelFormatter(new CustomLabelFormatter() {
			        					...
			        					animation.setStartOffset(0);
			        					ln.removeAllViews();
			        					MainActivity.graphs.set(facebook2.graphPos, graphView);
			        					ln.addView(MainActivity.graphs.get(facebook2.graphPos));

			        					MainActivity.graphs.get(facebook2.graphPos).startAnimation(animation);
			        					if(!facebook2.isPlay)
			        						mHandler.post(mTimer2);
										...
										public void onClick(View v) {
					        				...
					        				else
					        				{
						    					...
						    					/*realtimeGraph sg = MainActivity.rgraph.get(graphPos2);
						        					...
						        					MainActivity.graphs.get(facebook2.graphPos).removeAllSeries();
						        					MainActivity.graphs.get(facebook2.graphPos).addSeries(exampleSeries); // data*/
						        ...
						        valueTV.setText(facebook2.stockName+" Realtime Graph");
						        valueTV.setId(5);
			...
			public void run() {
				...
				AsyncTask<Void, Void, Void> updater = new AsyncTask<Void, Void, Void>() {
					...
					protected Void doInBackground(Void... arg0) {
						if(facebook2!=null && !facebook2.isPlay)
						{
						HttpURLConnection con = null;
						String payload = "Error";
						int numDays=0;
						...
						try {
						      ...
						      // Build RESTful query (GET)
						      URL url = new URL("http://finance.yahoo.com/d/quotes?f=sl1d1t1v&s="+facebook.stockAbrev);
						      //Log.d("url", "http://ichart.finance.yahoo.com/table.txt?a="+bmonth+"&b="+bday+"&c="+byear+"&d="+emonth+"&e="+eday+"&f="+eyear+"&g=d&s="+MainActivity.sgraph.get(i).stockAbrev);
						      con = (HttpURLConnection) url.openConnection();
						      con.setReadTimeout(10000);
						      con.setConnectTimeout(15000);
						      ...
						      while((payload = reader.readLine())!=null)
						      {
						    		  ...
						    		  if(val>facebook2.maxY)
						    			  facebook2.maxY=val;
						    		  if(val<facebook2.minY || facebook2.minY==0)
						    			  facebook2.minY=val;
					...
					protected void onPostExecute(Void result) {
						...
						if(facebook2!=null && !facebook2.isPlay)
						{
							//Log.d("point", Long.toString(facebook2.points.get(facebook2.points.size()-1).second)+"-"+Double.toString(facebook2.points.get(facebook2.points.size()-1).first));
						
						
							if(response && MainActivity.graphs.size()>facebook2.graphPos)
							 {
								graphView2.appendData(new GraphViewData(facebook2.points.get(facebook2.points.size()-1).second ,facebook2.points.get(facebook2.points.size()-1).first), true, 1000);
								facebook2.gvs=graphView2;
								MainActivity.graphs.get(facebook2.graphPos).setManualYAxisBounds(facebook2.maxY, facebook2.minY);
								facebook2.pos=iter-5;
							}
						 }
					}
