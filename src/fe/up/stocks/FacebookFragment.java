				...
				if(!exists)
				  ...
				  if(facebook.points.size()<1)
				  {
				    ...
				    try {
				          ...
				          URL url = new URL("http://ichart.finance.yahoo.com/table.txt?a="+bmonth+"&b="+bday+"&c="+byear+"&d="+emonth+"&e="+eday+"&f="+eyear+"&g=d&s="+facebook.stockAbrev);
										...
										public void onClick(View v) {
											if(facebook2.isPlay)
											 ...
											 if(!facebook2.isPlay){
												 	...
												 	MainActivity.rgraph.set(graphPos,facebook2);
											       }
											       else{
				...
				AsyncTask<Void, Void, Void> updater = new AsyncTask<Void, Void, Void>() {
					...
					protected void onPostExecute(Void result) {
						...
						Log.d("point", Long.toString(facebook2.points.get(facebook2.points.size()-1).second)+"-"+Double.toString(facebook2.points.get(facebook2.points.size()-1).first));
							...
							if(response)
							 {
								graphView2.appendData(new GraphViewData(facebook2.points.get(facebook2.points.size()-1).second ,facebook2.points.get(facebook2.points.size()-1).first), true, 31);
								facebook2.gvs=graphView2;
