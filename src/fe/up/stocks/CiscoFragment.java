				...
				if(!exists)
				  ...
				  if(cisco.points.size()<1)
				  {
				    ...
				    try {
				          ...
				          URL url = new URL("http://ichart.finance.yahoo.com/table.txt?a="+bmonth+"&b="+bday+"&c="+byear+"&d="+emonth+"&e="+eday+"&f="+eyear+"&g=d&s="+cisco.stockAbrev);
										...
										public void onClick(View v) {
											if(cisco2.isPlay)
											 ...
											 if(!cisco2.isPlay){
												 	...
												 	cisco2.isPlay=true;
												 	MainActivity.rgraph.set(graphPos,cisco2);
											       }
											       else{
											    	   cisco2.isPlay=false;
				...
				AsyncTask<Void, Void, Void> updater = new AsyncTask<Void, Void, Void>() {
					...
					protected void onPostExecute(Void result) {
						...
						Log.d("point", Long.toString(cisco2.points.get(cisco2.points.size()-1).second)+"-"+Double.toString(cisco2.points.get(cisco2.points.size()-1).first));
							...
							if(response)
							 {
								graphView2.appendData(new GraphViewData(cisco2.points.get(cisco2.points.size()-1).second ,cisco2.points.get(cisco2.points.size()-1).first), true, 31);
								cisco2.gvs=graphView2;
