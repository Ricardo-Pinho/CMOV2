				  ...
				  if(hpq.points.size()<1)
				  {
				    ...
				    try {
				    	  ...
				    	  if(MainActivity.monthPeriodGraph)
				    		  url = new URL("http://ichart.finance.yahoo.com/table.txt?a="+bmonth+"&b="+bday+"&c="+byear+"&d="+emonth+"&e="+eday+"&f="+eyear+"&g=d&s="+hpq.stockAbrev);
				    	  else
					          url = new URL("http://ichart.finance.yahoo.com/table.txt?a="+bmonth+"&b="+bday+"&c="+byear+"&d="+emonth+"&e="+eday+"&f="+eyear+"&g=m&s="+hpq.stockAbrev);
						...
						default:
						{
							...
							graphView.setCustomLabelFormatter(new CustomLabelFormatter() {
								...
								public String formatLabel(double value, boolean isValueX) {
									if (isValueX) {
										if(hpq.monthPeriod)
											return dateFormat.format(hpq.times.get((int) value));
										else
											return dateYearFormat.format(hpq.times.get((int) value));
									}
									else{
										double d= value;
							...
							LL.addView(LL2);
						        ...
						        btnR4.setOnClickListener(new OnClickListener() {
									...
									public void onClick(View v) {
									    ...
									    graphView2 = new GraphViewSeries(data);
									    graphView2.getStyle().color = Color.RED;
									    hpq2.points= new ArrayList<Pair<Double,Long>>();
									    MainActivity.rgraph.set(graphPos2,hpq2);
									    hpq2.maxY=(double) hpq.points.get(hpq.points.size()-1).first;
										hpq2.minY=(double) 0;
										hpq2.times=new ArrayList<String>();
										//MainActivity.graphs.get(hpq2.graphPos).removeAllSeries();
				    					//MainActivity.graphs.get(hpq2.graphPos).addSeries(graphView2);
									...
									graphView.setCustomLabelFormatter(new CustomLabelFormatter() {
			        					...
			        					animation.setStartOffset(0);
			        					ln.removeAllViews();
			        					MainActivity.graphs.set(hpq2.graphPos, graphView);
			        					ln.addView(MainActivity.graphs.get(hpq2.graphPos));

			        					MainActivity.graphs.get(hpq2.graphPos).startAnimation(animation);
			        					if(!hpq2.isPlay)
			        						mHandler.post(mTimer2);
				...
				AsyncTask<Void, Void, Void> updater = new AsyncTask<Void, Void, Void>() {
					...
					protected void onPostExecute(Void result) {
						...
						if(hpq2!=null && !hpq2.isPlay)
						{
							...
							if(response && MainActivity.graphs.size()>hpq2.graphPos)
							 {
								graphView2.appendData(new GraphViewData(hpq2.points.get(hpq2.points.size()-1).second ,hpq2.points.get(hpq2.points.size()-1).first), true, 1000);
								hpq2.gvs=graphView2;
