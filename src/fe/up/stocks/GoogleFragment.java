...
public class GoogleFragment extends Fragment {
	...
	private stockGraph google;
	private realtimeGraph google2;
			...
			protected View doInBackground(View... params) {
				...
				for(int i=0; i< MainActivity.sgraph.size();i++)
				{
					if(sg.stockName.equals(MainActivity.sgraph.get(i).stockName))
					{
						...
						{
							exists=true;
							google = MainActivity.sgraph.get(i);
							//google2 = MainActivity.sgraph.get(i);
							graphPos=i;
							break;
						}
					}
				...
				if(!exists)
				  ...
				  if(google.points.size()<1)
				  {
				    ...
				    try {
				    	  ...
				    	  int bmonth = google.beginDate.get(Calendar.MONTH);
				    	  int bday = google.beginDate.get(Calendar.DAY_OF_MONTH);
				    	  int byear = google.beginDate.get(Calendar.YEAR);
				    	  
				    	  int emonth = google.endDate.get(Calendar.MONTH);
				    	  int eday = google.endDate.get(Calendar.DAY_OF_MONTH);
				    	  int eyear = google.endDate.get(Calendar.YEAR);
				          // Build RESTful query (GET)
				    	  URL url;
				    	  if(MainActivity.monthPeriodGraph)
				    		  url = new URL("http://ichart.finance.yahoo.com/table.txt?a="+bmonth+"&b="+bday+"&c="+byear+"&d="+emonth+"&e="+eday+"&f="+eyear+"&g=d&s="+google.stockAbrev);
				    	  else
					          url = new URL("http://ichart.finance.yahoo.com/table.txt?a="+bmonth+"&b="+bday+"&c="+byear+"&d="+emonth+"&e="+eday+"&f="+eyear+"&g=m&s="+google.stockAbrev);
					...
					switch(response) {
						...
						default:
						{
							...
							for (int j=0; j<position; j++) {
								long x=google.points.get(j).second;
								double y=google.points.get(j).first;
								data[k] = new GraphViewData(x, y); // next day
								k--;
							}
							totaldiff = (google.maxY-google.minY)/3;
							...
							seriesStyle1.setValueDependentColor(new ValueDependentColor() {
								...
								public int get(GraphViewDataInterface data) {
									if(data.getY()<= (google.minY+totaldiff*1))
										return Color.rgb(0, 50, 150);
									else if(data.getY()<= (google.minY+totaldiff*2))
							...
							graphView.setManualYAxisBounds(google.maxY, google.minY);
							/*
							...
							graphView.setCustomLabelFormatter(new CustomLabelFormatter() {
								...
								public String formatLabel(double value, boolean isValueX) {
									if (isValueX) {
										if(google.monthPeriod)
											return dateFormat.format(google.times.get((int) value));
										else
											return dateYearFormat.format(google.times.get((int) value));
									}
									else{
										double d= value;
							...
							MainActivity.graphs.add(graphView);
							google.graphPos=MainActivity.graphs.size()-1;
							LL3.addView(MainActivity.graphs.get(MainActivity.graphs.size()-1));
									...
									public void onClick(View v) {
										...
										View parent = (View)p.getParent();
					    					stockGraph sg = MainActivity.sgraph.get(graphPos);
					        					...
					        					ln.removeAllViews();
					        					MainActivity.graphs.set(google.graphPos,graphView);
					        					ln.addView(MainActivity.graphs.get(google.graphPos));

					        					MainActivity.graphs.get(google.graphPos).startAnimation(animation);
					        				}
					        				//Log.i("test", "got here2");
									}
						        });
					        ...
					        title.setText(google.stockName);
							...
							LL.addView(LL2);
								...
								{
									if(google.stockName.equals(MainActivity.rgraph.get(i).stockName))
									{
										{
											exists=true;
											google2 = MainActivity.rgraph.get(i);
											//google2 = MainActivity.sgraph.get(i);
											graphPos2=i;
											graphView2 = google2.gvs;
								...
								if(!exists)
									{
										...
										/*for (int j=0; j<31; j++) {
											long x=0;
											double y=google.points.get(google.points.size()-1).first;
											google2.points.add(new Pair<Double,Long>(y,x));
										}*/
										data = new GraphViewData[0];
										//position=5;
										google2.pos=0;
										/*for (int j=0; j<31; j++) {
											long x=google2.points.get(j).second;
											double y=google2.points.get(j).first;
											data[j] = new GraphViewData(x, y); // next day
											//k--;
										}*/
									    graphView2 = new GraphViewSeries(data);
									    graphView2.getStyle().color = Color.RED;
									    google2.maxY=(double) google.points.get(google.points.size()-1).first;
										google2.minY=(double) 0;
									}

								// graph with custom labels and drawBackground
									graphView = new LineGraphView(
											getActivity()
											, ""/*google2.stockName+" Realtime Graph"*/
									);
									//((LineGraphView) graphView).setDrawBackground(true);
									
									
								graphView.addSeries(graphView2); // data
								google2.gvs = graphView2;
								MainActivity.rgraph.set(graphPos2, google2);
								graphView.setViewPort(google2.pos, 4);
								...
								graphView.setManualYAxisBounds(google2.maxY, google2.minY);
								
								graphView.setCustomLabelFormatter(new CustomLabelFormatter() {
									@Override
									public String formatLabel(double value, boolean isValueX) {
										if (isValueX) {
											if(value<0 || value>=google2.times.size())
												{
													Log.d(Double.toString(value),Integer.toString(google2.times.size()));
													return "0:00";
												}
											else
												return google2.times.get((int) value);
										}
										else{
													double d= value;
													String text = Double.toString(Math.abs(d));
								...
								MainActivity.graphs.add(graphView);
								google2.graphPos=MainActivity.graphs.size()-1;
						        ...
						        btnR4.setOnClickListener(new OnClickListener() {
									...
									public void onClick(View v) {
									    ...
									    graphView2 = new GraphViewSeries(data);
									    graphView2.getStyle().color = Color.RED;
									    google2.points= new ArrayList<Pair<Double,Long>>();
									    MainActivity.rgraph.set(graphPos2,google2);
									    google2.maxY=(double) google.points.get(google.points.size()-1).first;
										google2.minY=(double) 0;
										google2.times=new ArrayList<String>();
										//MainActivity.graphs.get(google2.graphPos).removeAllSeries();
				    					//MainActivity.graphs.get(google2.graphPos).addSeries(graphView2);
									...
									graphView.setCustomLabelFormatter(new CustomLabelFormatter() {
			        					...
			        					animation.setStartOffset(0);
			        					ln.removeAllViews();
			        					MainActivity.graphs.set(google2.graphPos, graphView);
			        					ln.addView(MainActivity.graphs.get(google2.graphPos));

			        					MainActivity.graphs.get(google2.graphPos).startAnimation(animation);
			        					if(!google2.isPlay)
			        						mHandler.post(mTimer2);
										...
										public void onClick(View v) {
					        				...
					        				else
					        				{
						    					...
						    					/*realtimeGraph sg = MainActivity.rgraph.get(graphPos2);
						        					...
						        					MainActivity.graphs.get(google2.graphPos).removeAllSeries();
						        					MainActivity.graphs.get(google2.graphPos).addSeries(exampleSeries); // data*/
						        ...
						        valueTV.setText(google2.stockName+" Realtime Graph");
						        valueTV.setId(5);
			...
			public void run() {
				...
				AsyncTask<Void, Void, Void> updater = new AsyncTask<Void, Void, Void>() {
					...
					protected Void doInBackground(Void... arg0) {
						if(google2!=null && !google2.isPlay)
						{
						HttpURLConnection con = null;
						String payload = "Error";
						int numDays=0;
						...
						try {
						      ...
						      // Build RESTful query (GET)
						      URL url = new URL("http://finance.yahoo.com/d/quotes?f=sl1d1t1v&s="+google.stockAbrev);
						      //Log.d("url", "http://ichart.finance.yahoo.com/table.txt?a="+bmonth+"&b="+bday+"&c="+byear+"&d="+emonth+"&e="+eday+"&f="+eyear+"&g=d&s="+MainActivity.sgraph.get(i).stockAbrev);
						      con = (HttpURLConnection) url.openConnection();
						      con.setReadTimeout(10000);
						      con.setConnectTimeout(15000);
						      ...
						      while((payload = reader.readLine())!=null)
						      {
						    		  ...
						    		  if(val>google2.maxY)
						    			  google2.maxY=val;
						    		  if(val<google2.minY || google2.minY==0)
						    			  google2.minY=val;
					...
					protected void onPostExecute(Void result) {
						...
						if(google2!=null && !google2.isPlay)
						{
							//Log.d("point", Long.toString(google2.points.get(google2.points.size()-1).second)+"-"+Double.toString(google2.points.get(google2.points.size()-1).first));
						
						
							if(response && MainActivity.graphs.size()>google2.graphPos)
							 {
								graphView2.appendData(new GraphViewData(google2.points.get(google2.points.size()-1).second ,google2.points.get(google2.points.size()-1).first), true, 1000);
								google2.gvs=graphView2;
								MainActivity.graphs.get(google2.graphPos).setManualYAxisBounds(google2.maxY, google2.minY);
								google2.pos=iter-5;
							}
						 }
					}
