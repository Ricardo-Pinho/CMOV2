				...
				if(!exists)
				  ...
				  if(oracle.points.size()<1)
				  {
				    ...
				    try {
				          ...
				          URL url = new URL("http://ichart.finance.yahoo.com/table.txt?a="+bmonth+"&b="+bday+"&c="+byear+"&d="+emonth+"&e="+eday+"&f="+eyear+"&g=d&s="+oracle.stockAbrev);
										...
										public void onClick(View v) {
											if(oracle2.isPlay)
											 ...
											 if(!oracle2.isPlay){
												 	...
												 	oracle2.isPlay=true;
												 	MainActivity.rgraph.set(graphPos,oracle2);
											       }
											       else{
				...
				AsyncTask<Void, Void, Void> updater = new AsyncTask<Void, Void, Void>() {
					...
					protected void onPostExecute(Void result) {
						...
						Log.d("point", Long.toString(oracle2.points.get(oracle2.points.size()-1).second)+"-"+Double.toString(oracle2.points.get(oracle2.points.size()-1).first));
							...
							if(response)
							 {
								graphView2.appendData(new GraphViewData(oracle2.points.get(oracle2.points.size()-1).second ,oracle2.points.get(oracle2.points.size()-1).first), true, 31);
								oracle2.gvs=graphView2;
