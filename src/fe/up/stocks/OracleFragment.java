...
public class OracleFragment extends Fragment {
	...
	private stockGraph oracle;
	private realtimeGraph oracle2;
			...
			protected View doInBackground(View... params) {
				...
				for(int i=0; i< MainActivity.sgraph.size();i++)
				{
					if(sg.stockName.equals(MainActivity.sgraph.get(i).stockName))
					{
						...
						{
							exists=true;
							oracle = MainActivity.sgraph.get(i);
							//oracle2 = MainActivity.sgraph.get(i);
							graphPos=i;
							break;
						}
					}
				...
				if(!exists)
				  ...
				  if(oracle.points.size()<1)
				  {
				    ...
				    try {
				    	  ...
				    	  int bmonth = oracle.beginDate.get(Calendar.MONTH);
				    	  int bday = oracle.beginDate.get(Calendar.DAY_OF_MONTH);
				    	  int byear = oracle.beginDate.get(Calendar.YEAR);
				    	  
				    	  int emonth = oracle.endDate.get(Calendar.MONTH);
				    	  int eday = oracle.endDate.get(Calendar.DAY_OF_MONTH);
				    	  int eyear = oracle.endDate.get(Calendar.YEAR);
				          // Build RESTful query (GET)
				    	  URL url;
				    	  if(MainActivity.monthPeriodGraph)
				    		  url = new URL("http://ichart.finance.yahoo.com/table.txt?a="+bmonth+"&b="+bday+"&c="+byear+"&d="+emonth+"&e="+eday+"&f="+eyear+"&g=d&s="+oracle.stockAbrev);
				    	  else
					          url = new URL("http://ichart.finance.yahoo.com/table.txt?a="+bmonth+"&b="+bday+"&c="+byear+"&d="+emonth+"&e="+eday+"&f="+eyear+"&g=m&s="+oracle.stockAbrev);
					...
					switch(response) {
						...
						default:
						{
							...
							for (int j=0; j<position; j++) {
								long x=oracle.points.get(j).second;
								double y=oracle.points.get(j).first;
								data[k] = new GraphViewData(x, y); // next day
								k--;
							}
							totaldiff = (oracle.maxY-oracle.minY)/3;
							...
							seriesStyle1.setValueDependentColor(new ValueDependentColor() {
								...
								public int get(GraphViewDataInterface data) {
									if(data.getY()<= (oracle.minY+totaldiff*1))
										return Color.rgb(0, 50, 150);
									else if(data.getY()<= (oracle.minY+totaldiff*2))
							...
							graphView.setManualYAxisBounds(oracle.maxY, oracle.minY);
							/*
							...
							graphView.setCustomLabelFormatter(new CustomLabelFormatter() {
								...
								public String formatLabel(double value, boolean isValueX) {
									if (isValueX) {
										if(oracle.monthPeriod)
											return dateFormat.format(oracle.times.get((int) value));
										else
											return dateYearFormat.format(oracle.times.get((int) value));
									}
									else{
										double d= value;
							...
							MainActivity.graphs.add(graphView);
							oracle.graphPos=MainActivity.graphs.size()-1;
							LL3.addView(MainActivity.graphs.get(MainActivity.graphs.size()-1));
									...
									public void onClick(View v) {
										...
										View parent = (View)p.getParent();
					    					stockGraph sg = MainActivity.sgraph.get(graphPos);
					        					...
					        					ln.removeAllViews();
					        					MainActivity.graphs.set(oracle.graphPos,graphView);
					        					ln.addView(MainActivity.graphs.get(oracle.graphPos));

					        					MainActivity.graphs.get(oracle.graphPos).startAnimation(animation);
					        				}
					        				//Log.i("test", "got here2");
									}
						        });
					        ...
					        title.setText(oracle.stockName);
							...
							LL.addView(LL2);
								...
								{
									if(oracle.stockName.equals(MainActivity.rgraph.get(i).stockName))
									{
										{
											exists=true;
											oracle2 = MainActivity.rgraph.get(i);
											//oracle2 = MainActivity.sgraph.get(i);
											graphPos2=i;
											graphView2 = oracle2.gvs;
								...
								if(!exists)
									{
										...
										/*for (int j=0; j<31; j++) {
											long x=0;
											double y=oracle.points.get(oracle.points.size()-1).first;
											oracle2.points.add(new Pair<Double,Long>(y,x));
										}*/
										data = new GraphViewData[0];
										//position=5;
										oracle2.pos=0;
										/*for (int j=0; j<31; j++) {
											long x=oracle2.points.get(j).second;
											double y=oracle2.points.get(j).first;
											data[j] = new GraphViewData(x, y); // next day
											//k--;
										}*/
									    graphView2 = new GraphViewSeries(data);
									    graphView2.getStyle().color = Color.RED;
									    oracle2.maxY=(double) oracle.points.get(oracle.points.size()-1).first;
										oracle2.minY=(double) 0;
									}

								// graph with custom labels and drawBackground
									graphView = new LineGraphView(
											getActivity()
											, ""/*oracle2.stockName+" Realtime Graph"*/
									);
									//((LineGraphView) graphView).setDrawBackground(true);
									
									
								graphView.addSeries(graphView2); // data
								oracle2.gvs = graphView2;
								MainActivity.rgraph.set(graphPos2, oracle2);
								graphView.setViewPort(oracle2.pos, 4);
								...
								graphView.setManualYAxisBounds(oracle2.maxY, oracle2.minY);
								
								graphView.setCustomLabelFormatter(new CustomLabelFormatter() {
									@Override
									public String formatLabel(double value, boolean isValueX) {
										if (isValueX) {
											if(value<0 || value>=oracle2.times.size())
												{
													Log.d(Double.toString(value),Integer.toString(oracle2.times.size()));
													return "0:00";
												}
											else
												return oracle2.times.get((int) value);
										}
										else{
													double d= value;
													String text = Double.toString(Math.abs(d));
								...
								MainActivity.graphs.add(graphView);
								oracle2.graphPos=MainActivity.graphs.size()-1;
						        ...
						        btnR4.setOnClickListener(new OnClickListener() {
									...
									public void onClick(View v) {
									    ...
									    graphView2 = new GraphViewSeries(data);
									    graphView2.getStyle().color = Color.RED;
									    oracle2.points= new ArrayList<Pair<Double,Long>>();
									    MainActivity.rgraph.set(graphPos2,oracle2);
									    oracle2.maxY=(double) oracle.points.get(oracle.points.size()-1).first;
										oracle2.minY=(double) 0;
										oracle2.times=new ArrayList<String>();
										//MainActivity.graphs.get(oracle2.graphPos).removeAllSeries();
				    					//MainActivity.graphs.get(oracle2.graphPos).addSeries(graphView2);
									...
									graphView.setCustomLabelFormatter(new CustomLabelFormatter() {
			        					...
			        					animation.setStartOffset(0);
			        					ln.removeAllViews();
			        					MainActivity.graphs.set(oracle2.graphPos, graphView);
			        					ln.addView(MainActivity.graphs.get(oracle2.graphPos));

			        					MainActivity.graphs.get(oracle2.graphPos).startAnimation(animation);
			        					if(!oracle2.isPlay)
			        						mHandler.post(mTimer2);
										...
										public void onClick(View v) {
					        				...
					        				else
					        				{
						    					...
						    					/*realtimeGraph sg = MainActivity.rgraph.get(graphPos2);
						        					...
						        					MainActivity.graphs.get(oracle2.graphPos).removeAllSeries();
						        					MainActivity.graphs.get(oracle2.graphPos).addSeries(exampleSeries); // data*/
						        ...
						        valueTV.setText(oracle2.stockName+" Realtime Graph");
						        valueTV.setId(5);
			...
			public void run() {
				...
				AsyncTask<Void, Void, Void> updater = new AsyncTask<Void, Void, Void>() {
					...
					protected Void doInBackground(Void... arg0) {
						if(oracle2!=null && !oracle2.isPlay)
						{
						HttpURLConnection con = null;
						String payload = "Error";
						int numDays=0;
						...
						try {
						      ...
						      // Build RESTful query (GET)
						      URL url = new URL("http://finance.yahoo.com/d/quotes?f=sl1d1t1v&s="+oracle.stockAbrev);
						      //Log.d("url", "http://ichart.finance.yahoo.com/table.txt?a="+bmonth+"&b="+bday+"&c="+byear+"&d="+emonth+"&e="+eday+"&f="+eyear+"&g=d&s="+MainActivity.sgraph.get(i).stockAbrev);
						      con = (HttpURLConnection) url.openConnection();
						      con.setReadTimeout(10000);
						      con.setConnectTimeout(15000);
						      ...
						      while((payload = reader.readLine())!=null)
						      {
						    		  ...
						    		  if(val>oracle2.maxY)
						    			  oracle2.maxY=val;
						    		  if(val<oracle2.minY || oracle2.minY==0)
						    			  oracle2.minY=val;
					...
					protected void onPostExecute(Void result) {
						...
						if(oracle2!=null && !oracle2.isPlay)
						{
							//Log.d("point", Long.toString(oracle2.points.get(oracle2.points.size()-1).second)+"-"+Double.toString(oracle2.points.get(oracle2.points.size()-1).first));
						
						
							if(response && MainActivity.graphs.size()>oracle2.graphPos)
							 {
								graphView2.appendData(new GraphViewData(oracle2.points.get(oracle2.points.size()-1).second ,oracle2.points.get(oracle2.points.size()-1).first), true, 1000);
								oracle2.gvs=graphView2;
								MainActivity.graphs.get(oracle2.graphPos).setManualYAxisBounds(oracle2.maxY, oracle2.minY);
								oracle2.pos=iter-5;
							}
						 }
					}
