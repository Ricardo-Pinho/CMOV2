				...
				for(int i=0; i< MainActivity.sgraph.size();i++)
				{
					if(sg.stockName.equals(MainActivity.sgraph.get(i).stockName))
					{
								...
								&& sg.endDate.YEAR == MainActivity.sgraph.get(i).endDate.YEAR)
						{
							...
							//ibm2 = MainActivity.sgraph.get(i);
							graphPos=i;
							break;
						}
					}
				...
				if(!exists)
				  ...
				  if(ibm.points.size()<1)
				  {
				    ...
				    try {
				    	  ...
				    	  int bmonth = ibm.beginDate.get(Calendar.MONTH);
				    	  int bday = ibm.beginDate.get(Calendar.DAY_OF_MONTH);
				    	  int byear = ibm.beginDate.get(Calendar.YEAR);
				    	  
				    	  int emonth = ibm.endDate.get(Calendar.MONTH);
					...
					switch(response) {
						...
						default:
						{
							...
							for (int j=0; j<position; j++) {
								long x=ibm.points.get(j).second;
								double y=ibm.points.get(j).first;
								data[k] = new GraphViewData(x, y); // next day
								k--;
							}
							totaldiff = (ibm.maxY-ibm.minY)/3;
							...
							seriesStyle1.setValueDependentColor(new ValueDependentColor() {
								...
								public int get(GraphViewDataInterface data) {
									if(data.getY()<= (ibm.minY+totaldiff*1))
										return Color.rgb(0, 50, 150);
									else if(data.getY()<= (ibm.minY+totaldiff*2))
							...
							graphView.setManualYAxisBounds(ibm.maxY, ibm.minY);
							/*
									...
									public void onClick(View v) {
										...
										View parent = (View)p.getParent();
					    					stockGraph sg = MainActivity.sgraph.get(graphPos);
					        					...
					        					ln.removeAllViews();
					        					MainActivity.graphs.set(ibm.graphPos,graphView);
					        					ln.addView(MainActivity.graphs.get(ibm.graphPos));
							...
							LL.addView(LL2);
								...
								{
									if(ibm.stockName.equals(MainActivity.rgraph.get(i).stockName))
									{
										{
											exists=true;
											ibm2 = MainActivity.rgraph.get(i);
											//ibm2 = MainActivity.sgraph.get(i);
											graphPos2=i;
											graphView2 = ibm2.gvs;
								...
								if(!exists)
									{
										...
										/*for (int j=0; j<31; j++) {
											long x=0;
											double y=ibm.points.get(ibm.points.size()-1).first;
											ibm2.points.add(new Pair<Double,Long>(y,x));
										}*/
								...
								MainActivity.graphs.add(graphView);
								ibm2.graphPos=MainActivity.graphs.size()-1;
						        ...
						        valueTV.setText(ibm2.stockName+" Realtime Graph");
						        valueTV.setId(5);
				...
				AsyncTask<Void, Void, Void> updater = new AsyncTask<Void, Void, Void>() {
					...
					protected Void doInBackground(Void... arg0) {
						...
						try {
						      ...
						      // Build RESTful query (GET)
						      URL url = new URL("http://finance.yahoo.com/d/quotes?f=sl1d1t1v&s="+ibm.stockAbrev);
						      //Log.d("url", "http://ichart.finance.yahoo.com/table.txt?a="+bmonth+"&b="+bday+"&c="+byear+"&d="+emonth+"&e="+eday+"&f="+eyear+"&g=d&s="+MainActivity.sgraph.get(i).stockAbrev);
						      con = (HttpURLConnection) url.openConnection();
						      con.setReadTimeout(10000);
						      con.setConnectTimeout(15000);
