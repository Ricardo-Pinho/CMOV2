...
public class AppleFragment extends Fragment {
	...
	private stockGraph apple;
	private realtimeGraph apple2;
			...
			protected View doInBackground(View... params) {
				...
				for(int i=0; i< MainActivity.sgraph.size();i++)
				{
					if(sg.stockName.equals(MainActivity.sgraph.get(i).stockName))
					{
						...
						{
							exists=true;
							apple = MainActivity.sgraph.get(i);
							//apple2 = MainActivity.sgraph.get(i);
							graphPos=i;
							break;
						}
					}
				...
				if(!exists)
				  ...
				  if(apple.points.size()<1)
				  {
				    ...
				    try {
				    	  ...
				    	  int bmonth = apple.beginDate.get(Calendar.MONTH);
				    	  int bday = apple.beginDate.get(Calendar.DAY_OF_MONTH);
				    	  int byear = apple.beginDate.get(Calendar.YEAR);
				    	  
				    	  int emonth = apple.endDate.get(Calendar.MONTH);
				    	  int eday = apple.endDate.get(Calendar.DAY_OF_MONTH);
				    	  int eyear = apple.endDate.get(Calendar.YEAR);
				          // Build RESTful query (GET)
				    	  URL url;
				    	  if(MainActivity.monthPeriodGraph)
				    		  url = new URL("http://ichart.finance.yahoo.com/table.txt?a="+bmonth+"&b="+bday+"&c="+byear+"&d="+emonth+"&e="+eday+"&f="+eyear+"&g=d&s="+apple.stockAbrev);
				    	  else
					          url = new URL("http://ichart.finance.yahoo.com/table.txt?a="+bmonth+"&b="+bday+"&c="+byear+"&d="+emonth+"&e="+eday+"&f="+eyear+"&g=m&s="+apple.stockAbrev);
					...
					switch(response) {
						...
						default:
						{
							...
							for (int j=0; j<position; j++) {
								long x=apple.points.get(j).second;
								double y=apple.points.get(j).first;
								data[k] = new GraphViewData(x, y); // next day
								k--;
							}
							totaldiff = (apple.maxY-apple.minY)/3;
							...
							seriesStyle1.setValueDependentColor(new ValueDependentColor() {
								...
								public int get(GraphViewDataInterface data) {
									if(data.getY()<= (apple.minY+totaldiff*1))
										return Color.rgb(0, 50, 150);
									else if(data.getY()<= (apple.minY+totaldiff*2))
							...
							graphView.setManualYAxisBounds(apple.maxY, apple.minY);
							/*
							...
							graphView.setCustomLabelFormatter(new CustomLabelFormatter() {
								...
								public String formatLabel(double value, boolean isValueX) {
									if (isValueX) {
										if(apple.monthPeriod)
											return dateFormat.format(apple.times.get((int) value));
										else
											return dateYearFormat.format(apple.times.get((int) value));
									}
									else{
										double d= value;
							...
							MainActivity.graphs.add(graphView);
							apple.graphPos=MainActivity.graphs.size()-1;
							LL3.addView(MainActivity.graphs.get(MainActivity.graphs.size()-1));
									...
									public void onClick(View v) {
										...
										View parent = (View)p.getParent();
					    					stockGraph sg = MainActivity.sgraph.get(graphPos);
					        					...
					        					ln.removeAllViews();
					        					MainActivity.graphs.set(apple.graphPos,graphView);
					        					ln.addView(MainActivity.graphs.get(apple.graphPos));

					        					MainActivity.graphs.get(apple.graphPos).startAnimation(animation);
					        				}
					        				//Log.i("test", "got here2");
									}
						        });
							...
							LL.addView(LL2);
								...
								{
									if(apple.stockName.equals(MainActivity.rgraph.get(i).stockName))
									{
										{
											exists=true;
											apple2 = MainActivity.rgraph.get(i);
											//apple2 = MainActivity.sgraph.get(i);
											graphPos2=i;
											graphView2 = apple2.gvs;
								...
								if(!exists)
									{
										...
										/*for (int j=0; j<31; j++) {
											long x=0;
											double y=apple.points.get(apple.points.size()-1).first;
											apple2.points.add(new Pair<Double,Long>(y,x));
										}*/
										data = new GraphViewData[0];
										//position=5;
										apple2.pos=0;
										/*for (int j=0; j<31; j++) {
											long x=apple2.points.get(j).second;
											double y=apple2.points.get(j).first;
											data[j] = new GraphViewData(x, y); // next day
											//k--;
										}*/
									    graphView2 = new GraphViewSeries(data);
									    graphView2.getStyle().color = Color.RED;
									    apple2.maxY=(double) apple.points.get(apple.points.size()-1).first;
										apple2.minY=(double) 0;
									}

								// graph with custom labels and drawBackground
									graphView = new LineGraphView(
											getActivity()
											, ""/*apple2.stockName+" Realtime Graph"*/
									);
									//((LineGraphView) graphView).setDrawBackground(true);
									
									
								graphView.addSeries(graphView2); // data
								apple2.gvs = graphView2;
								MainActivity.rgraph.set(graphPos2, apple2);
								graphView.setViewPort(apple2.pos, 4);
								...
								graphView.setManualYAxisBounds(apple2.maxY, apple2.minY);
								
								graphView.setCustomLabelFormatter(new CustomLabelFormatter() {
									@Override
									public String formatLabel(double value, boolean isValueX) {
										if (isValueX) {
											if(value<0 || value>=apple2.times.size())
												{
													Log.d(Double.toString(value),Integer.toString(apple2.times.size()));
													return "0:00";
												}
											else
												return apple2.times.get((int) value);
										}
										else{
													double d= value;
													String text = Double.toString(Math.abs(d));
								...
								MainActivity.graphs.add(graphView);
								apple2.graphPos=MainActivity.graphs.size()-1;
						        ...
						        btnR4.setOnClickListener(new OnClickListener() {
									...
									public void onClick(View v) {
									    ...
									    graphView2 = new GraphViewSeries(data);
									    graphView2.getStyle().color = Color.RED;
									    apple2.points= new ArrayList<Pair<Double,Long>>();
									    MainActivity.rgraph.set(graphPos2,apple2);
									    apple2.maxY=(double) apple.points.get(apple.points.size()-1).first;
										apple2.minY=(double) 0;
										apple2.times=new ArrayList<String>();
										//MainActivity.graphs.get(apple2.graphPos).removeAllSeries();
				    					//MainActivity.graphs.get(apple2.graphPos).addSeries(graphView2);
									...
									graphView.setCustomLabelFormatter(new CustomLabelFormatter() {
			        					...
			        					animation.setStartOffset(0);
			        					ln.removeAllViews();
			        					MainActivity.graphs.set(apple2.graphPos, graphView);
			        					ln.addView(MainActivity.graphs.get(apple2.graphPos));

			        					MainActivity.graphs.get(apple2.graphPos).startAnimation(animation);
			        					if(!apple2.isPlay)
			        						mHandler.post(mTimer2);
										...
										public void onClick(View v) {
					        				...
					        				else
					        				{
						    					...
						    					/*realtimeGraph sg = MainActivity.rgraph.get(graphPos2);
						        					...
						        					MainActivity.graphs.get(apple2.graphPos).removeAllSeries();
						        					MainActivity.graphs.get(apple2.graphPos).addSeries(exampleSeries); // data*/
						        ...
						        valueTV.setText(apple2.stockName+" Realtime Graph");
						        valueTV.setId(5);
				...
				AsyncTask<Void, Void, Void> updater = new AsyncTask<Void, Void, Void>() {
					...
					protected Void doInBackground(Void... arg0) {
						...
						try {
						      ...
						      // Build RESTful query (GET)
						      URL url = new URL("http://finance.yahoo.com/d/quotes?f=sl1d1t1v&s="+apple.stockAbrev);
						      //Log.d("url", "http://ichart.finance.yahoo.com/table.txt?a="+bmonth+"&b="+bday+"&c="+byear+"&d="+emonth+"&e="+eday+"&f="+eyear+"&g=d&s="+MainActivity.sgraph.get(i).stockAbrev);
						      con = (HttpURLConnection) url.openConnection();
						      con.setReadTimeout(10000);
						      con.setConnectTimeout(15000);
						      ...
						      while((payload = reader.readLine())!=null)
						      {
						    		  ...
						    		  if(val>apple2.maxY)
						    			  apple2.maxY=val;
						    		  if(val<apple2.minY || apple2.minY==0)
						    			  apple2.minY=val;
					...
					protected void onPostExecute(Void result) {
						...
						if(apple2!=null && !apple2.isPlay)
						{
							//Log.d("point", Long.toString(apple2.points.get(apple2.points.size()-1).second)+"-"+Double.toString(apple2.points.get(apple2.points.size()-1).first));
						
						
							if(response && MainActivity.graphs.size()>apple2.graphPos)
							 {
								graphView2.appendData(new GraphViewData(apple2.points.get(apple2.points.size()-1).second ,apple2.points.get(apple2.points.size()-1).first), true, 1000);
								apple2.gvs=graphView2;
								MainActivity.graphs.get(apple2.graphPos).setManualYAxisBounds(apple2.maxY, apple2.minY);
								apple2.pos=iter-5;
							}
						 }
					}
